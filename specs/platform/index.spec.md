...
Cluster registration, placement strategy, and cross-cluster session/gateway routing. The `Cluster` Kind models registered clusters with roles (`gateway`, `sandbox`). A physical cluster serving both roles is registered as two rows. The `PlacementStrategy` interface determines which cluster a session runs on (default: round-robin). Gateways can be deployed to dedicated gateway clusters while sessions run on sandbox clusters. See [data-model.spec.md § Cluster](data-model.spec.md#cluster--multi-cluster-scheduling) and [control-plane.spec.md § ClusterReconciler](control-plane.spec.md#internal-reconciler-cluster_reconcilergo--clusterreconciler).
