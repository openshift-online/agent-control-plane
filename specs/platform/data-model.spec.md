...
| `providers` | Nullable. JSONB array of provider names bound to this agent (e.g. `["vertex", "github"]`). References provider declarations in the same namespace. The control plane resolves provider secrets and configures gateway providers at session start. Not propagated to Session. |
