...
- **Gateway provisioning** — ~~the OpenShell gateway is assumed to already be deployed in each project namespace~~ **Resolved:** Gateway provisioning is now specified in [openshell-gateway.spec.md](./openshell-gateway.spec.md). Gateways are declared as `kind: Gateway` API resources via `acpctl apply -k` and reconciled by the GatewayReconciler in `internal/reconciler/`
...
The gateway MAY be exposed outside the cluster via a GRPCRoute (Gateway API) or NLB passthrough Route (ROSA/AWS) — see [`openshell-gateway-routing.spec.md`](./openshell-gateway-routing.spec.md) for exposure strategies. When exposed, external clients (openshell CLI) authenticate via OIDC Bearer tokens. When not exposed, the only clients are ACP (via mTLS + K8s SA token or OIDC) and sandboxes (via mTLS + gateway-minted JWTs).
