...
### Integration Credential Providers (replaces Credential Sidecars)

> **Note:** Credential sidecars have been replaced by the OpenShell provider model. Integration credentials are now injected into sandboxes via OpenShell gateway providers. The provider model achieves the same isolation guarantee — the agent process never holds real credentials, only opaque placeholders resolved by the gateway's egress proxy at request time. See `openshell-sandbox-provisioning.spec.md` § Credential Mapping to OpenShell Providers for the full provider mapping and lifecycle.

For each credential bound to the session's Project, the CP ensures an OpenShell
provider exists on the gateway. The gateway's egress proxy resolves credential
placeholders to real values at request time — the agent process inside the sandbox
never holds real credentials. Provider updates take effect immediately for subsequent
requests without restarting any sandbox.

When no credentials are bound to the Project, no credential providers are configured.
...
- **Push commits**: MCP server → `PushFiles` tool (commits and pushes in one call)
- **Create PRs**: MCP server → `CreatePullRequest` tool
...
### Sandbox Layout (Gateway Mode)

> **Note:** The credential sidecar pod layout has been replaced by the OpenShell gateway provider model. The sandbox layout below reflects the current architecture.

```
OpenShell Sandbox (session-{id})
├── runner process (started via ExecSandbox)
│     Environment:
...
│     Credentials injected via gateway egress proxy (opaque placeholders)
│
├── ambient-mcp sidecar
│     image: localhost/acp_mcp:latest
...
│
└── Gateway providers (configured per-project, not per-sandbox)
      - github provider    (if github credential bound)
      - vertex provider    (if vertex credential bound)
      - jira provider      (if jira credential bound, generic type)
