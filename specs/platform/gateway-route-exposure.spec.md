...
4. **gRPC compatibility.** OpenShift's HAProxy ingress controller supports HTTP/2 and gRPC over re-encrypt routes. The Route SHALL carry an annotation to enable HTTP/2 backend negotiation.
5. **Compatible with OIDC.** PR 363 disables client certificate (mTLS) verification when OIDC is enabled. Re-encrypt does not support mTLS, which is acceptable since OIDC replaces mTLS for authentication. Passthrough is the only mode supporting mTLS, but mTLS is not needed here.
...
The Gateway resource SHALL support an optional `route` field that declares external exposure via an OpenShift Route. When `route` is present, the GatewayReconciler SHALL create and reconcile a Route resource in the project namespace.
