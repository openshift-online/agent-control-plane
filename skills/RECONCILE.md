...
| U11 | platform-feedback | BFF feedback route (GET + POST) | FE | **done** | major | `POST /api/feedback` with SSO auth, input validation (3000 char cap, pagePath sanitization), Slack Block Kit delivery. `GET /api/feedback` availability check. |
| U12 | platform-feedback | Submission states (success/error) | FE | **done** | major | Loading spinner, success auto-close (2.5s), error preserves text. |
...
| U15 | platform-feedback | Rate limiting | FE | **removed** | minor | Removed: authenticated internal tool, no meaningful abuse vector. |
...
| 2026-07-28 | f9e85c56 | Wave 18 executed: U9–U16 (platform-feedback) | 92.4% | New spec `platform-feedback.spec.md` (8 reqs). Full implementation: BFF route (`GET`+`POST /api/feedback`), SSO auth, input validation, Slack Block Kit delivery, `FeedbackStrip` component (right-edge, z-9999, Shadcn Button), `FeedbackDialog` (shadcn Dialog), React Query hooks, `FEEDBACK_SLACK_WEBHOOK_URL` env var. Rate limiting removed post-review (authenticated internal tool). Lockfile regenerated for Node 22 container compat. |
