...
import (
	...
	"github.com/openshift-online/agent-control-plane/components/ambient-control-plane/internal/gateway"
...
func TestCredentialSidecarsGating_GatewayMode(t *testing.T) {
	tests := []struct {
		...
		{
			name:                "tokens configured — gateway handles credentials",
			cpTokenURL:          "http://cp:8080",
			cpTokenPublicKey:    "test-key",
			shouldBuildSidecars: false,
		},
		{
			name:                "missing token URL — gateway handles credentials",
			cpTokenURL:          "",
			cpTokenPublicKey:    "test-key",
			shouldBuildSidecars: false,
		},
...
func TestUseMCPSidecar_GatewayModeDisablesMCP(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "MCP image configured — gateway handles MCP",
			mcpImage:       "mcp:latest",
