...
import { FileCode, Tags, Shield } from 'lucide-react'
...
export default function PolicyDetailPage() {
  const { projectId, policyId } = useParams<{
    projectId: string
    policyId: string
  }>()
  const [activeTab, setActiveTab] = useState('manifest')
  const { data: policy, isLoading, error } = usePolicy(projectId, policyId)
  ...
  const annotationEntries = Object.entries(policy.annotations)
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
            <span>
              Namespace:{' '}
              <span className="font-mono">{policy.namespace}</span>
            </span>
            {sectionKeys.length > 0 && (
              <span>{sectionKeys.length} section(s)</span>
            )}
            {policy.updatedAt && (
              <span>Updated {formatRelativeTime(policy.updatedAt)}</span>
            )}
          </div>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        ...
        <TabsContent value="manifest">
          <PolicyManifestTab policy={policy} />
        </TabsContent>
